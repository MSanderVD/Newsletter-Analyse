...
# ---------------------------------------------------------------------------
# Gemini AI (kostenlos bis 1.500 Req/Tag)
# ---------------------------------------------------------------------------
...
def analyse_with_gemini(emails: list[dict], period_label: str) -> str:
    ...
    prompt = f"""Du analysierst Newsletter-Emails aus dem Zeitraum: {period_label}.

Hier sind {len(emails)} Emails:
...
1. Überschrift mit Zeitraum und Anzahl analysierter Emails
2. Top-10-Themenliste (nummeriert, absteigend nach Häufigkeit):
   - Themenname (prägnant, 2–5 Wörter)
   - Häufigkeit (Anzahl Emails, die das Thema erwähnen)
   - Kurze Beschreibung (1–2 Sätze)
   - Beispiel-Newsletter/Absender
3. Kurze Zusammenfassung der wichtigsten Trends (3–5 Sätze)
